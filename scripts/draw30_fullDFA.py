...
'''
model_name = "20241207-seed1-fullDFA"
total_test_times = 20
...
for i in range(1, total_test_times+1):
    ...
    total_DFA_data = pd.concat([total_DFA_data, data])
...
sns.lineplot(data=total_DFA_data, x='frames', y='rreturn_mean', label='mental_reward')
sns.lineplot(data=total_DFA_data, x='frames', y='return_mean', label="real_reward")
plt.xlabel('frames')
plt.ylabel('Reward')
