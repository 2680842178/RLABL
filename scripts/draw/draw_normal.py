...
ABL_model_name = "20250107-discover-DQN-easy-large"
DFA_model_name = "20240112-DFA-DQN-easy-large"
fig_name="easy-large-DQN"
...
for i in range(1, total_test_times+1):
    ...
    RL_merged_data = RL_merged_data[RL_merged_data['frames'] < 301500]
    DFA_merged_data = DFA_data[['frames', 'rreturn_mean', 'return_mean']]
    DFA_merged_data = DFA_merged_data[DFA_merged_data['frames'] < 301500]
...
sns.lineplot(data=total_ABL_data, x='frames', y='return_mean', label='RLABL(D3QN)', color='salmon', linewidth=1.5, alpha=0.8)
sns.lineplot(data=total_RL_data, x='frames', y='return_mean', label='RL(D3QN)', color='steelblue', linewidth=1.5, alpha=0.8)
sns.lineplot(data=total_DFA_data, x='frames', y='return_mean', label='DFA(D3QN)', color='mediumseagreen', linewidth=1.5, alpha=0.8)
