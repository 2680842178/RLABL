...
BASE_MODEL_NAME="20250112-RL-PPO-taxi" # 设置模型名称
...
for i in $(seq 1 1); do
  ...
  # 修改任务配置并执行训练
  python discover_anomaly.py --task-config task1 --discover 0 --algo $ALGO --env $ENV --lr $LR  --model $MODEL_NAME --discount $DISCOUNT --epochs $EPOCHS --frames-per-proc $FRAMES_PER_PROC --frames $TOTAL_STEPS --seed $i --curriculum 1  --contrast $CONTRAST_FUNC
done
