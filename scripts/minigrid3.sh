...
for i in $(seq 1 30); do
  # 设置模型名称和配置文件夹
  MODEL_NAME="20241225-minigrid-discover-dqn-${i}"
  ...
  sed -i "${START_LINE},${END_LINE}d" $CONFIGMAP
  printf "%s\n" "$MAP_2" >> $CONFIGMAP
  python discover_anomaly.py --task-config task1 --discover 1 --algo $ALGO --env MiniGrid-ConfigWorld-v0-havekey --lr $LR --AnomalyNN test_8 --model $MODEL_NAME --discount $DISCOUNT --epochs $EPOCHS --frames-per-proc $FRAMES_PER_PROC --frames 300000
  # add key to the map
  sed -i "${START_LINE},${END_LINE}d" $CONFIGMAP
  printf "%s\n" "$MAP_3" >> $CONFIGMAP
