...
BASE_MODEL_NAME="20250107-discover-DQN-easy-large" # 设置模型名称
...
DISCOVER_STEPS=300000 # discover过程的最多步数，注意这步数是算在总步数里的，所以最好小于单个课程训练的步数。
CONTRAST_FUNC="SSIM"
###################################
...
for i in $(seq 1 $NUMS); do
  ...
  # 修改任务配置并执行训练
  python discover_anomaly.py --task-config task1 --discover 0 --algo $ALGO --env $ENV --lr $LR  --model $MODEL_NAME --discount $DISCOUNT --seed $i --epochs $EPOCHS --frames-per-proc $FRAMES_PER_PROC --frames $CURRICULUM_1_STEPS --configmap $CONFIGMAP --curriculum 1 --discover-steps $DISCOVER_STEPS --contrast $CONTRAST_FUNC  
  if [ $? -gt 4 ]; then
    echo "Error during task 1, stopping the script."
    exit 1
  fi
  python discover_anomaly.py --task-config task1 --discover 1 --algo $ALGO --env $ENV --lr $LR  --model $MODEL_NAME --discount $DISCOUNT --seed $i --epochs $EPOCHS --frames-per-proc $FRAMES_PER_PROC --frames $CURRICULUM_2_STEPS --configmap $CONFIGMAP --curriculum 2 --discover-steps $DISCOVER_STEPS --contrast $CONTRAST_FUNC  
  if [ $? -gt 4 ]; then
    echo "Error during task 2, stopping the script."
    exit 1
  fi

  python discover_anomaly.py --task-config task2 --discover 1 --algo $ALGO --env $ENV --lr $LR --model $MODEL_NAME --discount $DISCOUNT --seed $i --epochs $EPOCHS --frames-per-proc $FRAMES_PER_PROC --frames $CURRICULUM_3_STEPS --configmap $CONFIGMAP --curriculum 3 --discover-steps $DISCOVER_STEPS --contrast $CONTRAST_FUNC  
  if [ $? -gt 4 ]; then
