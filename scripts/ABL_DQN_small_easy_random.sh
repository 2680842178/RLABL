...
BASE_MODEL_NAME="20250110-discover-DQN-small-easy-random" # 设置模型名称
...
for i in $(seq 1 $NUMS); do
  ...
  # 修改任务配置并执行训练
  CUDA_VISIBLE_DEVICES=$DEVICE_ID python discover_anomaly.py --task-config task1 --discover 0 --algo $ALGO --env $ENV --lr $LR  --model $MODEL_NAME --discount $DISCOUNT --epochs $EPOCHS --frames-per-proc $FRAMES_PER_PROC --frames $CURRICULUM_1_STEPS --seed $i --configmap $CONFIGMAP --curriculum 1 --discover-steps $DISCOVER_STEPS --contrast $CONTRAST_FUNC --fixed $FIXED
  if [ $? -gt 4 ]; then
    echo "Error during task 1, stopping the script."
    exit 1
  fi
  CUDA_VISIBLE_DEVICES=$DEVICE_ID python discover_anomaly.py --task-config task1 --discover 1 --algo $ALGO --env $ENV --lr $LR  --model $MODEL_NAME --discount $DISCOUNT --epochs $EPOCHS --frames-per-proc $FRAMES_PER_PROC --frames $CURRICULUM_2_STEPS --seed $i --configmap $CONFIGMAP --curriculum 2 --discover-steps $DISCOVER_STEPS --contrast $CONTRAST_FUNC --fixed $FIXED
  if [ $? -gt 4 ]; then
    echo "Error during task 2, stopping the script."
    exit 1
  fi

  CUDA_VISIBLE_DEVICES=$DEVICE_ID python discover_anomaly.py --task-config task2 --discover 1 --algo $ALGO --env $ENV --lr $LR --model $MODEL_NAME --discount $DISCOUNT --epochs $EPOCHS --frames-per-proc $FRAMES_PER_PROC --frames $CURRICULUM_3_STEPS --seed $i --configmap $CONFIGMAP --curriculum 3 --discover-steps $DISCOVER_STEPS --contrast $CONTRAST_FUNC --fixed $FIXED
  if [ $? -gt 4 ]; then
