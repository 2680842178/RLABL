...
# 设置模型名称和配置文件夹
MODEL_NAME=20241226-discover-ppo-changemap
...
sed -i "${START_LINE},${END_LINE}d" $CONFIGMAP
printf "%s\n" "$MAP_3" >> $CONFIGMAP
python discover_anomaly.py --task-config task2 --discover 1 --algo $ALGO --env MiniGrid-ConfigWorld-v0 --lr $LR --AnomalyNN test_8 --model $MODEL_NAME --discount $DISCOUNT --epochs $EPOCHS --frames-per-proc $FRAMES_PER_PROC --frames 900000
