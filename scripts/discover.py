...
def define_accept_mutation(
    mutation_score, mutation_times, test_turns, test_mean_reward
):
    score = (
        0.4 * mutation_score
        + 0.0001 * mutation_times
        + 0.01 * test_turns
        + (1 + test_mean_reward["mean"]) / 2
    )
    if score > 0.6:
        return True
    return False
...
def contrast(
    mutation1, mutation2
) -> float:  # that means the similarity between two mutations
...
def test(
    ...
    test_logs = {"num_frames_per_episode": [], "return_per_episode": []}
    env: gym.Env = copy_env(start_env, args.env)
    mutation_buffer = []
    ...
    for _ in range(episodes):
        for i in range(max_steps_per_episode):
            ...
            with torch.no_grad():
                dist, _ = G.nodes[current_state]["state"].agent.acmodel(
                    preprocessed_obss
                )
            ...
            )  # tuple(a | b for a, b in zip(terminateds, truncateds))
...
def discover(
    ...
    while num_frames < discover_frames:
        ...
        exps, logs1 = collect_experiences_mutation(
            algo,
            start_env,
            get_mutation_score,
            mutation_buffer,
        ...
        if update % args.log_interval == 0:
            ...
            txt_logger.info(
                "U {} | F {:06} | FPS {:04.0f} | D {} | Reward:μσmM {:.2f} {:.2f} {:.2f} {:.2f} |  entropy {:.3f} | value {:.3f} | policy_loss {:.3f} | value_loss {:.3f} | grad_norm {:.3f}".format(
                    *data
                )
            )
    ...
    for node in G.nodes:
        if list(G.predecessors(node)):
            if node != 0 and node != 1:
                G.nodes[node]["state"].mutation = plt.imread(
                    task_path + "/mutation" + str(node) + ".bmp"
                )
        if node != 0 and node != 1:
            G.nodes[node]["state"].env_image = plt.imread(
                state_img_path + "/state" + str(node) + ".bmp"
            )
    ...
    if args.discover != 0:
        ...
        for node, data in G.nodes(data=True):
            if data["state"].env_image is not None:
                node_probability_list[node] = contrast(
                    data["state"].env_image, initial_img
                )
        ...
        min_stop_state = 0
        while 1:
            test_start_node = numpy.random.choice(
                len(node_probability_list), 1, p=node_probability_list
            )[0]
            test_turns = 10
            mean_return, _, stop_state, stop_env, stop_obss = test(
                envs[0], test_start_node, 10, 128, preprocess_obss
            )
            if stop_state > min_stop_state:
                min_stop_state = stop_state
            if mean_return["mean"] > 0.8:
                txt_logger.info(
                    "successful test! Start from: {0}, reward per episode: {1}".format(
                        test_start_node, mean_return
                    )
                )
                return

            txt_logger.info(
                "Test {} turns results: Start from {}, reward per episode: {}".format(
                    test_turns, test_start_node, mean_return
                )
            )
            ...
            if random_number <= discover_prob:
                new_mutation, new_state_img, out_state = discover(
                    start_env=stop_env,
                    ...
                    # anomalyNN=AnomalyNN,
                    MutationMudule=mutation_module,
                )
                if new_mutation is not None:
                    ...
                    G.add_node(
                        new_node_id,
                        state=stateNode(
                            new_node_id, None, None, stop_env.gen_obs()["image"]
                        ),
                    )
                    if min_stop_state == start_node:
                        ...
                        plt.imsave(
                            state_img_path + "state{}.bmp".format(new_node_id),
                            initial_img,
                        )
                    ...
                    for successor in G.successors(new_node_id):
                        new_yaml["graph"]["edges"].append(
                            {"from": new_node_id, "to": successor}
                        )
                    for predecessor in G.predecessors(new_node_id):
                        new_yaml["graph"]["edges"].append(
                            {"from": predecessor, "to": new_node_id}
                        )
                    ...
                    for node in G.nodes:
                        if G.nodes[node]["state"].mutation is not None:
                            plt.imsave(
                                new_task_path + "/mutation" + str(node) + ".bmp",
                                G.nodes[node]["state"].mutation,
                            )
    ...
    while num_frames < args.frames:
        ...
        if args.algo == "a2c" or args.algo == "ppo":
            exps_list, logs1, statenn_exps = Mutiagent_collect_experiences(
                ...
                # anomalyNN=AnomalyNN,
                mutation_module=mutation_module,
        ...
        if args.test_interval > 0 and update % args.test_interval == 0:
            test_return_per_episode, test_num_frames_per_episode, _, _, _ = test(
                envs[0], start_node, 10, 256, preprocess_obss
            )
            print(start_node)
            txt_logger.info(
                "U {} | Test reward:μσmM {:.2f} {:.2f} {:.2f} {:.2f} | Test num frames:μσmM {:.2f} {:.2f} {:.2f} {:.2f}".format(
                    10,
                    *(test_return_per_episode.values()),
                    *(test_num_frames_per_episode.values()),
                )
            )
