...
BASE_MODEL_NAME="DFA-PPO-taxi" # 设置模型名称
...
for i in $(seq 1 $NUMS); do
  ...
  cp -r ./config/DFA-ABL-PPO-taxi-config/* ./config/$MODEL_NAME

  # 修改任务配置并执行训练
  /home/sporeking/miniconda3/envs/py312/bin/python discover_anomaly.py --task-config task1 --discover 0 --algo $ALGO --env $ENV --lr $LR  --model $MODEL_NAME --discount $DISCOUNT --epochs $EPOCHS --frames-per-proc $FRAMES_PER_PROC --frames $TOTAL_STEPS --seed $i --curriculum 1 --discover-steps $DISCOVER_STEPS --contrast $CONTRAST_FUNC
done
