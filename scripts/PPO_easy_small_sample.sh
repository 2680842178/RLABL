...
for i in $(seq 1 $NUMS); do
  ...
  # 修改任务配置并执行训练
  CUDA_VISIBLE_DEVICES=$DEVICE_ID python sample_exp.py --task-config task1 --discover 0 --algo $ALGO --env $ENV --lr $LR --model $MODEL_NAME --discount $DISCOUNT --epochs $EPOCHS --frames-per-proc $FRAMES_PER_PROC --frames $TOTAL_STEPS --seed $i --configmap $CONFIGMAP --curriculum 3
  if [ $? -gt 4 ]; then
