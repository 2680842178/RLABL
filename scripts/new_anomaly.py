...
class ConvAutoencoder(nn.Module):
    def __init__(self, dropout_prob):
        ...
        self.enc5 = nn.Conv2d(256, 512, kernel_size=4, stride=2, padding=1)  # 增加编码层
        ...
        self.dec5 = nn.ConvTranspose2d(32, 3, kernel_size=4, stride=2, padding=1)  # 增加解码层
...
class MutationModule:
    ...
    def add_to_buffer(self, img):
        img.save(f'self.normal_dir/{len([f for f in os.listdir(normal_dir)])}.bmp', format='BMP')

    def train(self, num_epochs):
        normal_dataset = ImageDataset(self.normal_dir, label=0, transform=self.transform)
        train_loader = DataLoader(normal_dataset, batch_size=params['batch_size'], shuffle=True)
        ...
        for epoch in range(num_epochs):
            ...
            # 使用核密度估计拟合分布
            self.reconstruction_errors = np.array(reconstruction_errors).reshape(-1, 1)  # 保存重构误差
            self.kde = KernelDensity(kernel='gaussian', bandwidth=0.5).fit(self.reconstruction_errors)
...
if len(normal_dataset) > 0 and len(anomaly_dataset) > 0:
    ...
    for idx, (score, prob) in enumerate(zip(anomaly_scores, anomaly_probabilities)):
        print(f"Image {idx + 1}: Reconstruction Error = {score[0]:.4f}, Anomaly Probability = {prob:.4f}")

    # 可视化重构误差的分布
    plt.figure(figsize=(10, 5))
    plt.hist(mutation_module.reconstruction_errors, bins=50, alpha=0.6, color='g', label='Normal Data')
    plt.hist([s[0] for s in anomaly_scores], bins=50, alpha=0.6, color='r', label='Anomaly Data')
