...
BASE_MODEL_NAME="20240104-PPO-difficult-large" # 设置模型名称
CONFIGMAP="difficult_large_maps.config" # 设置地图文件:
...
# 设置三个课程的总步数（累加关系）
CURRICULUM_1_STEPS=1000000
CURRICULUM_2_STEPS=2000000
CURRICULUM_3_STEPS=3000000
###################################
...
for i in $(seq 1 10); do
  ...
  # 修改任务配置并执行训练
  CUDA_VISIBLE_DEVICES=$DEVICE_ID python discover_anomaly.py --task-config task1 --discover 0 --algo $ALGO --env $ENV --lr $LR  --model $MODEL_NAME --discount $DISCOUNT --epochs $EPOCHS --frames-per-proc $FRAMES_PER_PROC --frames $CURRICULUM_1_STEPS --seed $i --configmap $CONFIGMAP --curriculum 1
  if [ $? -gt 4 ]; then
    echo "Error during task 1, stopping the script."
    exit 1
  fi
  CUDA_VISIBLE_DEVICES=$DEVICE_ID python discover_anomaly.py --task-config task1 --discover 0 --algo $ALGO --env $ENV --lr $LR  --model $MODEL_NAME --discount $DISCOUNT --epochs $EPOCHS --frames-per-proc $FRAMES_PER_PROC --frames $CURRICULUM_2_STEPS --seed $i --configmap $CONFIGMAP --curriculum 2
  if [ $? -gt 4 ]; then
