...
def read_maps_from_config(config_file='random_maps.config'):
    ...
    for i in range(1, 61):
        map_str = config['random_maps'][f"map{i}"]
...
class RandomMinigridEnv(MiniGridEnv):
    ...
    def _gen_grid(self, width, height):
        ...
        if self.curriculum == 1:
            selected_map = random.choice(self.maps[:5])
...
class RandomMinigridEnvHavekey(MiniGridEnv):
    ...
    def __init__(self, size=8,config_path='random_maps.config', max_steps: int = 256, **kwargs):
        self.size=size
        # 设置最大步数
        if max_steps is None:
            max_steps = 20 * self.size
...
def copy_env(copied_env, env_key, seed=None, render_mode="rgb_array", curriculum=1, **kwargs):
    ...
    if env_key == "MiniGrid-ConfigWorld-Random" or env_key == "MiniGrid-ConfigWorld-Random-havekey":
        curriculum = copied_env.curriculum
    # new_env = gym.make(env_key, render_mode=render_mode)
    new_env = make_env(env_key, seed=seed, render_mode=render_mode, curriculum=curriculum, **kwargs)
